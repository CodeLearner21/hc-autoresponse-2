...
def sidebar():
    with st.sidebar:
        st.image('cloudbeds_logo.png', width=200)
        ...
        with st.chat_message(message["role"]):
            if message["role"] == "assistant" and "image_paths" in message:
                for path in message["image_paths"]:
                    ...
                    except (requests.RequestException, FileNotFoundError, OSError, MediaFileStorageError) as e:
...
def main():
    hotel_name = sidebar()
    hotel_email = 'stay@mercurehydepark.com'
